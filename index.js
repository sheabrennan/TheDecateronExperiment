...
prompts([
  ...
  const game = new Store(gameName.game, { base: './.config/games' })
  if (!game.get('name')) {
...
function generate (name, init, rooms) {
  ...
  const build = new Store(name, { base: './.config/games' }, init.get('initer'))
...
async function play (game, initGame = false) {
  ...
  switch (resp.location) {
    ...
    case 'close':
      close()
      display()
      console.log('Current: ' + gameDetails.currentCell + '-' + gameDetails.currentRoom)
      break
    case 'trace':
...
`)
    ...
    Gravity: ${gameDetails.gravity < 0 ? room.gravity.desc : (rooms[gameDetails.currentRoom].gravity.type)}
    Description: ${room.description}
    Bad Guys: ${room.badguys}
    
          `)

    if (gameDetails.currentCell == gameDetails.exitCell && gameDetails.currentRoom == gameDetails.exitRoom) {
  ...
  function trace (count = 20000, checkVisited = false) {
    ...
    for (let t = count; t > 0; t--) {
      ...
      const currentPath = gameDetails.gameLog.slice(0, (gameDetails.gameLog.length - (originalLog.length + 1)))
  ...
  async function preview () {
    ...
    const respChoices = gameDetails.currentDoors.map((d, i) => {
      ...
      const nextRoom = cells[nextCell].linkedCells[gameDetails.currentRoom].doors[i]
      ...
      if (gameDetails.exitCell == nextCell && gameDetails.exitRoom == nextRoom) {
        door.title += '  (EXIT!!!)'
      }
    ...
    console.log(`currC: ${gameDetails.currentCell}  currR ${gameDetails.currentRoom}`)
    console.log(gameDetails.shortestPath)
